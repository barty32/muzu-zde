...
async function getKontejnerData(type: KontejnerType): Promise<POI[]> {
	...
	for (const feature of data.features) {
		...
		const coords = convertCoords3857((feature.geometry as Point).coordinates);
		const point: POI = { id, coords, type, extraData: { ...feature.properties, type } };
		result.push(point);
	}
	return result;
}
...
function isPointInPolygon(polygon: Position[], point: LatLng): boolean {
	...
	for (let i = 0, j = polygon.length - 1; i < polygon.length; i++) {
		...
		if (((polygon[i][1] > point.lat) !==
			(polygon[j][1] >
				point
					.lat))  // One point needs to be above, one below our y coordinate
...
export function getFullNameOfZoneType(zone_type: string): string {
	switch (zone_type) {
		case 'skolske_zarizeni':
			return 'Školské zařízení';
		case 'zastavka_mhd':
			return 'Zastávka MHD';
		case 'zdravotni_zarizeni':
			return 'Zdravotní zařízení';
	}
	throw 'Invalid input';
}